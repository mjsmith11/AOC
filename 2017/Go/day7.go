...
func day7() {
	nodes, children := getGraph()
	for _, v := range children["marnqj"] {
		fmt.Print(v)
		fmt.Print(" ")
		fmt.Println(getNodeWeight(nodes, children, v))
	}
}

func getNodeWeight(nodes map[string]int, children map[string][]string, node string) int {
	if value, exists := children[node]; exists {
		sum := nodes[node]
		for _, n := range value {
			sum = sum + getNodeWeight(nodes, children, n)
		}
		return sum
	}
	return nodes[node]
...
//returns map[node]weight, map[node]children
func getGraph() (map[string]int, map[string][]string) {
	scanner := bufio.NewScanner(strings.NewReader(getInput7()))
	nodes := make(map[string]int, 100)
	edges := make(map[string][]string, 100)

	for scanner.Scan() {
		parenPieces := strings.Split(scanner.Text(), "(")
		nodeName := strings.Trim(parenPieces[0], " ")
		nodeName = strings.Trim(nodeName, "\t")
		weight, _ := strconv.Atoi(strings.Split(parenPieces[1], ")")[0])
		nodes[nodeName] = weight

		if strings.Contains(scanner.Text(), "->") {
			pieces := strings.Split(scanner.Text(), "->")
			toNodes := strings.Trim(pieces[1], " ")
			edges[nodeName] = strings.Split(toNodes, ", ")
		}
	}
...
func getRoot() string {
	...
	for scanner.Scan() {
		if strings.Contains(scanner.Text(), "->") {
			pieces := strings.Split(scanner.Text(), "(")
			nodes[strings.Trim(pieces[0], " ")] = ""
			//fmt.Println(strings.Trim(pieces[0], " "))
		}
	}
	...
	for scanner.Scan() {
		if strings.Contains(scanner.Text(), "->") {
			pieces := strings.Split(scanner.Text(), "->")
			toNodes := strings.Trim(pieces[1], " ")
			nodeList := strings.Split(toNodes, ", ")
...
func getInput7() string {
	return `uglvj (99) -> ymfjt, gkpgf
	vvwrg (51)
	qrpgt (5)
	qhqbqj (55)
	taxdaf (23)
