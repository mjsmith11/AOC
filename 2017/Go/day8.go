...
var day8Reg map[string]int

func day8() (int, int) {
	day8Reg = make(map[string]int, 100)
	runningMax := 0
	scanner := bufio.NewScanner(strings.NewReader(getInput8()))
	for scanner.Scan() {
		line := scanner.Text()
		splits := strings.Split(line, " ")
		reg := splits[0]
		op := splits[1]
		amt, _ := strconv.Atoi(splits[2])
		creg := splits[4]
		cop := splits[5]
		cconst, _ := strconv.Atoi(splits[6])
		if doCondition(creg, cop, cconst) {
			if op == "inc" {
				setReg(reg, getReg(reg)+amt)
			}
			if op == "dec" {
				setReg(reg, getReg(reg)-amt)
			}
			if v := getReg(reg); v > runningMax {
				runningMax = v
			}
		}
	}
	...
	endMaxVal := 0
	for _, v := range day8Reg {
		if v > endMaxVal {
			endMaxVal = v
		}
	}

	return endMaxVal, runningMax
}

func getReg(name string) int {
	if v, e := day8Reg[name]; e {
		return v
	}
	day8Reg[name] = 0
	return 0
...
func setReg(name string, value int) {
	day8Reg[name] = value
}

func doCondition(reg string, operator string, constant int) bool {
	regValue := getReg(reg)
	if operator == "==" {
		return regValue == constant
	}
	if operator == "<" {
		return regValue < constant
	}
...
func getInput8() string {
	return `d dec 683 if qn == 0
d dec -220 if h == 0
