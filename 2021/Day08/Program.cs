...
static int part1() {
    ...
    foreach(string s in input) {
        ...
        string[] chars = splits[1].Trim().Split(" ");
        foreach(string c in chars) {
            if (c.Length == 2) { count++; } // 1's
            if (c.Length == 4) { count++; } // 4's
            if (c.Length == 3) { count++; } // 7's
            if (c.Length == 7) { count++; } // 8's
        }
    }
    return count;
}
...
static int part2() {
    ...
    foreach(string s in input) {
        ...
        // loop again to use the knowledge from the easy ones to work on the harder ones
        foreach(string str in chars) {
            string c = String.Concat(str.OrderBy(c => c));
            if (c.Length == 6) { // this is the 0, 6, and 9
                // only 9 should have all segments from the 4
                bool is9 = true;
                foreach(char ch in reverseMap[4]) {
                    if (!c.Contains(ch)) { is9 = false; }
                }
                if (is9) {
                    map[c] = 9;
                    reverseMap[9] = c;
                } else {
                    // if it's not 9 and has everything from the 1, it must be 0
                    bool is0 = true;
                    foreach(char ch in reverseMap[1]) {
                        if (!c.Contains(ch)) { is0 = false; }
                    }
                    if (is0) {
                        map[c] = 0;
                        reverseMap[0] = c;
                    } else {
                        //it's 6.  9 & 0 have been ruled out
                        map[c] = 6;
                        reverseMap[6] = c;
                    }
                }
            }
            if (c.Length == 5) { // this is the 2, 3 and 5
                // 3 is the only one that should completely match the 1
                bool is3 = true;
                foreach(char ch in reverseMap[1]) {
                    if (!c.Contains(ch)) { is3 = false; }
                }
                if (is3) {
                    map[c] = 3;
                    reverseMap[3] = c;
                } else {
                    // count matches with 4.  2 will have 2 and 5 will have 3
                    int fourmatches = 0;
                    foreach(char ch in reverseMap[4]) {
                        if (c.Contains(ch)) { fourmatches++; }
                    }
                    if (fourmatches==2){
                        map[c] = 2;
                        reverseMap[2] = c;
        ...
        //should know all the numbers, so we can 
        string[] output = splits[1].Trim().Split(" ");
        string sortedplace0 = String.Concat(output[0].OrderBy(c => c));
        string sortedplace1 = String.Concat(output[1].OrderBy(c => c));
        string sortedplace2 = String.Concat(output[2].OrderBy(c => c));
        string sortedplace3 = String.Concat(output[3].OrderBy(c => c));

        sum += (1000*map[sortedplace0]);
        sum += (100*map[sortedplace1]);
        sum += (10*map[sortedplace2]);
        sum += (map[sortedplace3]);
    }
    return sum;
}
...
static string[] getInput() {
    string data = @"gfbd geadcb gaefc agdcf cdgfba dcf abdefc cadbg agfecbd df | acgbd dafcg fd df
