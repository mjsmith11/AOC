...
[1518-06-06 23:56] Guard #3271 begins shift";
               ...
               if (e.desc.StartsWith("Guard"))
                {
                    ...
                    guards.Add(int.Parse(descWords[1].Split('#')[1]));
                }
            }
            ...
            foreach(int guard in guards)
            {
                ...
                int maxFreq = 0;
                int maxFreqMin = 0;
                foreach(KeyValuePair<int, int> a in freqs)
                {
                    if (a.Value > maxFreq)
                    {
                        maxFreq = a.Value;
                ...
                myResult.guard = guard;
                myResult.minute = maxFreqMin;
                myResult.freq = maxFreq;
                r.Add(myResult);
            }
            ...
            {
                if(res.freq > frequency)
                {
                    frequency = res.freq;
                    guardChoice = res.guard;
                    minChoice = res.minute;
                }
            }
            Console.WriteLine(guardChoice * minChoice);
            Console.ReadLine();
        }
