...
namespace ConsoleApp1
{
    class Program
    {
        ...
        {
            string input = @"[1518-10-31 00:58] wakes up
...
[1518-06-06 23:56] Guard #3271 begins shift";
            ...
            foreach(string s in splits)
            {
                ...
                string[] timestampParts = parts2[1].Split(' ');
                ...
                Event e = new Event();
                e.desc = parts[1].Trim();
                e.timestamp = new DateTime(year, month, day, hour, min, 0);
                events.Add(e);
            }
            List<Event> sortedEvents = events.OrderBy(o => o.timestamp).ToList();
            Dictionary<int, int> asleepMins = new Dictionary<int, int>();
            int currentGuard = 0;
            int fellAsleepMin = 0;

            foreach(Event e in sortedEvents)
            {
               if (e.desc.StartsWith("Guard"))
                {
                    string[] descWords = e.desc.Split(' ');
                    currentGuard = int.Parse(descWords[1].Split('#')[1]);
                }
               else if (e.desc.StartsWith("falls asleep"))
                {
                    fellAsleepMin = e.timestamp.Minute;
                }
               else if(e.desc.StartsWith("wakes up"))
                {
                    int minsAsleep = e.timestamp.Minute - fellAsleepMin;
                    if (asleepMins.ContainsKey(currentGuard))
                    {
                        asleepMins[currentGuard] += minsAsleep;
                    }
                    else
                    {
                        asleepMins[currentGuard] = minsAsleep;
            ...
            int maxMinsAsleep = 0;
            int guard = 0;
            foreach (KeyValuePair<int, int> a in asleepMins){
                if (a.Value > maxMinsAsleep)
                {
                    maxMinsAsleep = a.Value;
                    guard = a.Key;
                }
            }

            Dictionary<int, int> minFreq = new Dictionary<int, int>();
            foreach (Event e in sortedEvents)
            {
                if (e.desc.StartsWith("Guard"))
                {
                ...
                else if (e.desc.StartsWith("wakes up"))
                {
                    ...
                    {
                        for(int i = fellAsleepMin; i< e.timestamp.Minute; i++)
                        {
                            if (minFreq.ContainsKey(i))
                            {
                                minFreq[i]++;
                            }
                            else
            ...
            int maxFreq = 0;
            int myMin = 0;
            foreach (KeyValuePair<int, int> a in minFreq)
            {
                if (a.Value > maxFreq)
                {
                    maxFreq = a.Value;
