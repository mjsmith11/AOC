...
namespace ConsoleApp1
{
	class Program
	{
		...
		{
			string input = @"position=< 41710,  52012> velocity=<-4, -5>
...
position=< 31320,  52012> velocity=<-3, -5>";
			...
			{
				string[] thisSplits = s.Split('<');

				string posInfo = thisSplits[1].Split('>')[0];
				string[] posSplits = posInfo.Split(',');
				Particle myPart = new Particle();
				myPart.x = int.Parse(posSplits[0].Trim());
				myPart.y = int.Parse(posSplits[1].Trim());

				string velInfo = thisSplits[2].Split('>')[0];
				string[] velSplits = velInfo.Split(',');

				myPart.vx = int.Parse(velSplits[0].Trim());
				myPart.vy = int.Parse(velSplits[1].Trim());
			...
			while (sec<10378)
			{
				...
				if (sec > 10374)
				{
					...
					int minY = parts.Min(o => o.y);
					int maxY = parts.Max(o => o.y);

					int minX = parts.Min(o => o.x);
					int maxX = parts.Max(o => o.x);

					for (int y = minY; y <= maxY; y++)
					{
						for (int x = minX; x <= maxX; x++)
						{
							Particle check = new Particle();
							check.x = x;
							check.y = y;
							if (parts.Contains(check, new ParticleComparer()))
							{
								Console.Write("#");
							}
							else
		...
		{
			public bool Equals(Particle a, Particle b)
			{
				return (a.x == b.x) && (a.y == b.y);
			}
			public int GetHashCode(Particle obj)
			{
				return obj.x + obj.y + obj.vx + obj.vy;
			}
		}
