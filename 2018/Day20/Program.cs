...
namespace ConsoleApp1
{
	class Program
	{
		static void Main(string[] args)
		{
			...
			foreach (char c in input)
			{
				if ("NESW".Contains(c))
				{
					List<Point> newPos = new List<Point>();
				...
				{
					PointListPair plp = new PointListPair();
					plp.starts = starts;
					plp.ends = ends;
					stack.Push(plp);
					starts = positions;
			...
			g.Dijkstra(new Point(0,0));

			Console.WriteLine("Part #1 " + g.dist.Values.Max());
			Console.WriteLine("Part #2 " + g.dist.Values.Where(o => o >= 1000).Count());
	...
	public class PointListPair{
		public List<Point> starts;
		public List<Point> ends;

		public PointListPair()
		{
	...
	public class Graph
	{
		private Dictionary<Point, List<Point>> matrix;
		public Dictionary<Point, int> dist;
		Dictionary<Point, Point> prev;

		public Graph()
		{
			matrix = new Dictionary<Point, List<Point>>();
		}
		public void addEdge(Point n1, Point n2)
		{
			addEdgeOneWay(n1, n2);
			addEdgeOneWay(n2, n1);
		}
		...
		{
			foreach (KeyValuePair<Point,List<Point>> kvp in matrix)
			{
				Console.Write("(" + kvp.Key.x + "," + kvp.Key.y + ") => ");
		...
		public void Dijkstra(Point start)
		{
			dist = new Dictionary<Point, int>();
			prev = new Dictionary<Point, Point>();
			List<Point> vertices = matrix.Keys.ToList();
			foreach (Point p in vertices)
			{
				dist[p] = int.MaxValue;
				prev[p] = new Point(int.MaxValue, int.MaxValue);
			}
			dist[start] = 0;
			while (vertices.Count() > 0)
			{
				Point myP = new Point(int.MinValue, int.MinValue);
		...
		private void addEdgeOneWay(Point n1, Point n2)
		{
			if(!matrix.ContainsKey(n1))
			{
				matrix[n1] = new List<Point>();				
			}
			if (!matrix[n1].Contains(n2))
			{
				matrix[n1].Add(n2);
			}
		}
