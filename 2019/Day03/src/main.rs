...
fn main() {
    ...
    for d in directions1 {
        ...
        for _ in 0..amt {
            ...
            steps += 1;
            area[[curx,cury]] = steps;
        }
    }

    // walk wire 2
    let directions2 = wire2.split(",");
    curx = centerx;
    cury = centery;
    let mut mindist = 10000000;
    let mut minsteps = 10000000;
    steps = 0;
    for d in directions2 {
        ...
        for _ in 0..amt {
            ...
            if area[[curx,cury]] > 0 {
                // part1 check distance from start
                let x : i32 = (curx as i32)-(centerx as i32);
                let y : i32 = (cury as i32)-(centery as i32);
                let dist = x.abs() + y.abs();
                if dist < mindist { mindist = dist;}

                // part2 check steps
                let total_steps = steps + area[[curx, cury]];
                if total_steps < minsteps { minsteps = total_steps;}
            }
        }
    }
