...
namespace Day15
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Point position = new Point(size/2, size/2);
            area[position.x, position.y] = '.';
            Computer c = new Computer(input);
            search(c,position,0);
            
            Console.WriteLine("Part 1: " + pathLengths.Min());
            
            //Part 2 -------------------------------------------------------------
            int oxygenNeeded = countChar(area, size, '.') + countChar(area, size, 'O');
            Point startLocation = findChar(area, size, 'O');
            int minutesPassed = 0;
            List<Point> oxygenToPropagate = new List<Point>();
            oxygenToPropagate.Add(startLocation);
            while (countChar(area,size,'O') != oxygenNeeded) {
                List<Point> nextRoundOxygen = new List<Point>();
                foreach(Point p in oxygenToPropagate) {
                    // north
                    if (area[p.x, p.y - 1] == '.') {
                        area[p.x, p.y - 1] = 'O';
                        nextRoundOxygen.Add(new Point(p.x, p.y - 1));
                    }
                    // south
                    if (area[p.x, p.y + 1] == '.') {
                        area[p.x, p.y + 1] = 'O';
                    ...
                    if (area[p.x - 1, p.y] == '.') {
                        area[p.x - 1, p.y] = 'O';
                        nextRoundOxygen.Add(new Point(p.x - 1, p.y));
                    }
                    // east
                    if (area[p.x + 1, p.y] == '.') {
                        area[p.x + 1, p.y] = 'O';
                ...
                oxygenToPropagate = nextRoundOxygen;
                minutesPassed++;
            }
            Console.WriteLine("Part 2: " + minutesPassed);
        }
        
        static int countChar(char[,] map, int size, char searchChar) {
            int count = 0;
            for(int x=0; x<size; x++) {
                for (int y = 0; y<size; y++){
                    if (map[x,y] == searchChar) {
                        count++;
                    }
                }
            }
        ...
        static Point findChar(char[,] map, int size, char searchChar) {
            if (countChar(map,size,searchChar) != 1) { Console.WriteLine("Character doesn't appear exactly once"); }
            for(int x=0; x<size; x++) {
                for (int y = 0; y<size; y++){
                    if (map[x,y] == searchChar) {
        ...
        // Recursive DFSish search thinger to create the map
        static void search(Computer droid, Point location, int steps) {
            // try to go north
            if (area[location.x, location.y - 1] == '?'){
                long[] response = droid.execute(1,0);
                steps++;
                if (response[0] == 0){
                    // hit a wall
                    area[location.x, location.y - 1] = '#';
                } else {
                    area[location.x, location.y - 1] = (response[0]==1?'.':'O');
                    if (response[0]==2) { pathLengths.Add(steps); }
                    location.y = location.y - 1;
                    search(droid, location, steps);
                    // move back
                    droid.execute(2,0);
                    location.y = location.y + 1;
                }
                steps--;
            } else if (area[location.x, location.y - 1] == 'O') {
                pathLengths.Add(steps + 1);
            }
            // try to go south
            if (area[location.x, location.y + 1] == '?'){
                long[] response = droid.execute(2,0);
                steps++;
                if (response[0] == 0){
                    // hit a wall
                    area[location.x, location.y + 1] = '#';
            ...
            // try to go west
            if (area[location.x - 1, location.y] == '?'){
                long[] response = droid.execute(3,0);
                steps++;
                if (response[0] == 0){
                    // hit a wall
                    area[location.x - 1, location.y] = '#';
                } else {
                    area[location.x - 1, location.y] = (response[0]==1?'.':'O');
                    if (response[0]==2) { pathLengths.Add(steps); }
                ...
                steps--;
            } else if (area[location.x - 1, location.y] == 'O') {
                pathLengths.Add(steps + 1);
            }
            // try to go east
            if (area[location.x + 1, location.y] == '?'){
                long[] response = droid.execute(4,0);
                steps++;
                if (response[0] == 0){
                    // hit a wall
                    area[location.x + 1, location.y] = '#';
