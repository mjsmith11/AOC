...
namespace Day11
{
    class Program
    {
        static void Main(string[] args)
        {
           ...
           hull[size/2,size/2] = '#'; // comment this for Part 1
            Computer c = new Computer(input);
            while (!c.halted) {
                Int64[] output = c.execute((hull[pos.x, pos.y]=='.'?0:1),0); // only using the 1st input
                //paint it
                hull[pos.x, pos.y] = (output[0]==0?'.':'#');
                painted.Add(pos.x * 1000 + pos.y);
                // turn
                dir = getNewDir(dir, (output[1]==0?'L':'R'));
                pos.move(dir);
            }
            Console.WriteLine("Part 1: " + painted.Count);
            Console.WriteLine("Part 2: ");
            // had to pull in the boundaries some to see the result on my terminal
            for (int y=25; y<size-25; y++) {
                for (int x=25; x<size-25; x++) {
        ...
        static char getNewDir(char curDir, char turnDir) {
            if (curDir=='U') {
                return turnDir;
            } else if (curDir == 'D') {
                if (turnDir == 'L') { return 'R'; }
                else { return 'L'; }
            } else if (curDir == 'R') {
                if (turnDir == 'L') { return 'U'; }
                else { return 'D'; }
            } else {
                if (turnDir == 'L') { return 'D'; }
                else { return 'U'; }
            }
        }
    }
