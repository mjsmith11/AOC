    ...
    class Program
    {
        ...
        static int part1() {
            ...
            foreach(string s in input) {
                ...
                string desc = words[0] + " " + words[1];
                string regex = @"(?<num>\d) (?<desc>[a-z]* [a-z]*) bag";
                List<string> contents = new List<string>();
                foreach(Match m in Regex.Matches(s, regex)) {
                    contents.Add(m.Groups["desc"].Value);
                }
                rules.Add(desc,contents);
            }

            int result = 0;
            foreach(KeyValuePair<string,List<string>> entry in rules)
            {
                string lookingFor = "shiny gold";
                if (entry.Key == lookingFor) { continue; } // this doesn't count.  We want to have the bag inside at least 1 other
                Queue<string> searchQueue = new Queue<string>();
                searchQueue.Enqueue(entry.Key);
                while(searchQueue.Count > 0) {
                    string current = searchQueue.Dequeue();
                    foreach(string bag in rules[current]) {
                        if (bag==lookingFor) {
                            result++;
                            searchQueue.Clear();
                            break;
                        } else {
                            searchQueue.Enqueue(bag);
                        }
                    }
                }
            }
        ...
        static int part2() {
            ...
            foreach(string s in input) {
                ...
                List<string> contents = new List<string>();
                foreach(Match m in Regex.Matches(s, regex)) {
                    contents.Add(m.Groups["num"].Value + " " + m.Groups["desc"].Value);
                }
                rules.Add(desc,contents);
            }

            Queue<string> searchQueue = new Queue<string>();
            searchQueue.Enqueue("shiny gold");
            while(searchQueue.Count > 0) {
                string current = searchQueue.Dequeue();
                foreach(string bag in rules[current]) {
                    string[] ruleWords = bag.Split(" ");
                    int num = int.Parse(ruleWords[0]);
                    string ruleDesc = ruleWords[1] + " " + ruleWords[2];
                    for(int i = 0; i<num; i++) {
                        bagsInside++;
                        searchQueue.Enqueue(ruleDesc);
                    }
                }
            }
        ...
        static string[] getInput() {
            string input = @"vibrant bronze bags contain 3 dim olive bags.
