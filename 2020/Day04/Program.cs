...
namespace Day04
{
    class Program
    {
        ...
        static int run(bool doPart2Validation) {
            string[] input = getInput();
            int line = 0;
            string regex = @"(?<key>([a-z])*):(?<value>([a-z]|\d|#)*)";
            int validPassports = 0;
            while(line<input.Length) {
                Dictionary<string,string> passportData = new Dictionary<string,string>();
                while((line<input.Length) && (!String.IsNullOrWhiteSpace(input[line]))) {
                    foreach(Match m in Regex.Matches(input[line], regex)) {
                        passportData.Add(m.Groups["key"].Value, m.Groups["value"].Value);
                    }
                    line++;
                }
                // check for validity
                bool valid = part1Validation(passportData);
                if (valid && doPart2Validation) {valid = part2Validation(passportData);}

                if (valid) {validPassports++;}


                // move on to the next passport
                line++;
            }
            return validPassports;
        }

        static bool part1Validation(Dictionary<string, string> data) {
            bool valid = data.ContainsKey("byr");
            valid &= data.ContainsKey("iyr");
            valid &= data.ContainsKey("eyr");
            valid &= data.ContainsKey("hgt");
            valid &= data.ContainsKey("hcl");
            valid &= data.ContainsKey("ecl");
            valid &= data.ContainsKey("pid");
            return valid;
        }

        static bool part2Validation(Dictionary<string, string> data) {
            string fourDigitRegex = "^[0-9]{4}$";
            // byr
            Match m = Regex.Match(data["byr"], fourDigitRegex);
            if (!m.Success) { return false;}
            int byr = int.Parse(data["byr"]);
            if (byr<1920 || byr>2002) { return false;}

            // iyr
            m = Regex.Match(data["iyr"], fourDigitRegex);
            if (!m.Success) { return false;}
            int iyr = int.Parse(data["iyr"]);
            if (iyr<2010 || iyr>2020) { return false;}

            //eyr
            m = Regex.Match(data["eyr"], fourDigitRegex);
            if (!m.Success) { return false;}
            int eyr = int.Parse(data["eyr"]);
            if (eyr<2020 || eyr>2030) { return false;}

            //hgt
            string hgtPattern = @"(?<num>[0-9]*)(?<unit>cm|in)";
            m = Regex.Match(data["hgt"], hgtPattern);
            if (!m.Success) { return false;}
            int hgt = int.Parse(m.Groups["num"].Value);
            string hgtUnit = m.Groups["unit"].Value;
            if(hgtUnit == "cm" && (hgt<150 || hgt>193)) { return false; }
            if(hgtUnit == "in" && (hgt<59 || hgt>76)) { return false; }

            //hcl
            string hclPattern = @"#([0-9a-f]){6}";
            m = Regex.Match(data["hcl"], hclPattern);
            if (!m.Success) { return false;}

            //ecl
            string eclPattern = @"^amb|blu|brn|gry|grn|hzl|oth$";
            m = Regex.Match(data["ecl"], eclPattern);
            if (!m.Success) { return false;}

            //pid
            string pidPattern = @"^[0-9]{9}$";
            m = Regex.Match(data["pid"], pidPattern);
            if (!m.Success) { return false;}
        ...
        static string[] getInput() {
            string input = @"byr:2010 pid:#1bb4d8 eyr:2021 hgt:186cm iyr:2020 ecl:grt
