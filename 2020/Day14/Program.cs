...
namespace Day14
{
    class Program
    {
        ...
        {
            /*string maskstr = "XXXXXXXXXXXXXXXXXXXXXXXXXXXXX1XXXX0X";
            ulong ormask = getOrMask(maskstr);
            ulong andmask = getAndMask(maskstr);
            ulong value = 0;
            ulong maskedValue = (value | ormask) & andmask;*/
        ...
        static ulong part1() {
            ...
            for(int i = 0; i<program.Length; i++) {
                ...
                if (splits[0] == "mask") {
                    ormask = getOrMask(splits[2].Trim());
                    andmask = getAndMask(splits[2].Trim());
                }
                else {
                    string regex = @"mem\[(?<key>\d+)\] = (?<value>\d+)";
                    Match m = Regex.Match(line,regex);
                    int key = int.Parse(m.Groups["key"].Value);
                    ulong value = ulong.Parse(m.Groups["value"].Value);
                    ulong setValue = (value | ormask) & andmask;
                    mem[key] = setValue;
                }
            }
            ulong sum = 0;
            foreach(KeyValuePair<int,ulong> entry in mem)
            {
                sum += entry.Value;
            }
            return sum;
        ...
        static ulong part2() {
            ...
            for(int i = 0; i<program.Length; i++) {
                ...
                if (splits[0] == "mask") {
                    ormask = getOrMask(splits[2].Trim());
                    // mask the floating bits (X) to 0 initially
                    andmask = getAndMask2(splits[2].Trim());
                    floats = getFloatingPlaces(splits[2].Trim());
                }
                else {
                    string regex = @"mem\[(?<key>\d+)\] = (?<value>\d+)";
                    Match m = Regex.Match(line,regex);
                    ulong key = ulong.Parse(m.Groups["key"].Value);
                    ulong value = ulong.Parse(m.Groups["value"].Value);
                    ulong maskedKey = (key | ormask) & andmask;
                    // floats is an array of bit values 1,2,4,8,16,32 for the floating bits
                    // this loop is a way to loop over all subsets of values in float.
                    // the current subset contains elements where the bit in j corresponding
                    // to it's index is a 1.
                    for(int j=0; j<(int)(Math.Pow(2,floats.Length)); j++) {
                        ulong myKey = maskedKey;
                        // create a key by adding in the bitvalues from float
                        // that have a 1 in the bit of j that corresponds to their index.
                        for(int k=0; k<floats.Length; k++) {
                            // checking if bit k of j is a 1
                            if ((j & (1 << k)) != 0) {
                                myKey += floats[k];
                            }
                        }
            ...
            ulong sum = 0;
            foreach(KeyValuePair<ulong,ulong> entry in mem)
            {
                sum += entry.Value;
            }
            return sum;
        ...
        static ulong getOrMask(string mask) {
            ulong bitmask = 0;
            for(int i = 0; i<mask.Length; i++) {
                int maskIndex = mask.Length - 1 - i; //need i ascending, but we need to work R to L
                if (mask[maskIndex] == '1') {
                    bitmask += (ulong)(Math.Pow(2,i));
                }
            }
            return bitmask;
        }

        static ulong getAndMask(string mask) {
            ulong bitmask = ulong.MaxValue;
            for(int i = 0; i<mask.Length; i++) {
                int maskIndex = mask.Length - 1 - i; //need i ascending, but we need to work R to L
                if (mask[maskIndex] == '0') {
                    bitmask -= (ulong)(Math.Pow(2,i));
        ...
        static ulong getAndMask2(string mask) {
            ulong bitmask = ulong.MaxValue;
            for(int i = 0; i<mask.Length; i++) {
                int maskIndex = mask.Length - 1 - i; //need i ascending, but we need to work R to L
                if (mask[maskIndex] == 'X') {
        ...
        static ulong[] getFloatingPlaces(string mask) {
            int size = 0;
        ...
        static string[] getInput() {
            string inp = @"mask = 00X10101X110010011XX0X011X100000X010
mem[13197] = 47579321
mem[37394] = 374943
mem[48509] = 418284
mem[29793] = 191792725
