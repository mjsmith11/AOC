...
namespace Day02
{
    class Program
    {
        ...
        static int part1() {
            ...
            foreach(string s in input) {
                ...
                foreach(char c in password) {
                    if (c == letter) { letterMatches++; }
                }
                if ((letterMatches>=min) && (letterMatches<=max)) {
                    valid++;
                }
            }
            return valid;
        ...
        static int part2() {
            string[] input = getInput();
            string pattern = @"(?<firstIndex>\d+)-(?<secondIndex>\d+) (?<letter>[a-z]): (?<password>[a-z]+)";
            int valid = 0;
            foreach(string s in input) {
                Match m = Regex.Match(s,pattern);
                int i1 = int.Parse(m.Groups["firstIndex"].Value) - 1; //subtract 1 because it's a 1 based index
                int i2 = int.Parse(m.Groups["secondIndex"].Value) - 1; //subtract 1 because it's a 1 based index
                char letter = m.Groups["letter"].Value[0];
                string password = m.Groups["password"].Value;
        ...
        static string[] getInput() {
            string input = @"15-16 l: klfbblslvjclmlnqklvg
