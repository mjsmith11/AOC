...
namespace ConsoleApplication4
{
	class Part1
	{
		static void Main(string[] args)
		{
			...
			int minWinCost = int.MaxValue;
			int minw, mina, minr1, minr2;
				...
				for (int a = 0; a < Armor.Count(); a++)
				{
					for (int r1 = 0; r1 < Rings.Count(); r1++)
					{
						for (int r2 = 0; r2 < Rings.Count(); r2++)
						{
							...
							if (playerWins(playerPoints, damage, armor, bossPoints, bossDamage, bossArmor) && cost < minWinCost)
							{
								minWinCost = cost;
								minw = w;
								mina = a;
