...
// Note: this doesn't solve part 1. I used Ctrl+F on the input page
...
func main() {
	scanner := bufio.NewScanner(strings.NewReader(getInput16()))
	for scanner.Scan() {
		text := strings.Trim(scanner.Text(), " ")
		parts := strings.Split(text, " ")
		attr := strings.Trim(parts[2], ":")
		number, _ := strconv.Atoi(strings.Trim(parts[3], ","))
		test1 := checkAttribute(attr, number)

		attr = strings.Trim(parts[4], ":")
		number, _ = strconv.Atoi(strings.Trim(parts[5], ","))
		test2 := checkAttribute(attr, number)

		attr = strings.Trim(parts[6], ":")
		number, _ = strconv.Atoi(strings.Trim(parts[7], ","))
		test3 := checkAttribute(attr, number)

		if test1 && test2 && test3 {
			fmt.Println("Sue " + strings.Trim(parts[1], ":"))
		}

	}
}

func checkAttribute(attribute string, number int) bool {
	if attribute == "children" {
		return number == 3
	} else if attribute == "cats" {
		return number > 7
	} else if attribute == "samoyeds" {
		return number == 2
	} else if attribute == "pomeranians" {
		return number < 3
	} else if attribute == "akitas" {
		return number == 0
	} else if attribute == "vizslas" {
		return number == 0
	} else if attribute == "goldfish" {
		return number < 5
	} else if attribute == "trees" {
		return number > 3
	} else if attribute == "cars" {
		return number == 2
	} else if attribute == "perfumes" {
		return number == 1
	}
	return false
}
func getInput16() string {
